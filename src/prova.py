...
for epoch in range(1000000):
    loss,loss_data,loss_pde,loss_bc = model.train_epoch(train_dataset,inputs_not_sampled,freq,optimizer,data_weights,pde_weights,bc_weights,points_sampled,pinn=pinn)
    val_loss = model.test_epoch(val_dataset)

    if epoch % 10 == 0:
        print(f'Epoch [{epoch}/{1000000}], Loss: {loss.item()},Val_Loss:{val_loss.item()}')
...
utils.plot_model(data_handler,previsions_pinn,points_sampled,freq[0,:],index,pinn)
...
# Estrai la parte reale e la parte immaginaria
real_parts = [z.real for z in data_handler.NORMALIZED_OUTPUT[:,index]]
imaginary_parts = [z.imag for z in data_handler.NORMALIZED_OUTPUT[:,index]]


real_parts_sampled = [z.real for z in data_handler.NORMALIZED_OUTPUT_SAMPLED[:,index]]
imaginary_parts_sampled= [z.imag for z in data_handler.NORMALIZED_OUTPUT_SAMPLED[:,index]]
# Crea un diagramma sul piano complesso
